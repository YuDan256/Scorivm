...
forma edita StatusReg {
    ...
    sit _: p8 mic 4;              // 匿名位域，用于占位填充 (Padding) 跳过 4 bits
}
```

### 9.2 语义与约束 (Regulae)

1. **允许的类型**：位域只能应用于标量整数类型（`i8`~`i64`, `p8`~`p64`）以及布尔类型（`logica`）。严禁用于浮点数、指针（`via`）、切片（`cohors`）或数组。
2. **内存布局 (Memory Layout)**：
   * 连续声明的位域会被打包（Packed）到同一个底层存储单元中。
   * **标准结构体 (`forma`)**：遵循 C 语言的对齐规则。如果下一个位域的宽度超出了当前存储单元的剩余位数，它会被推到下一个对齐边界，中间留下空白（Padding）。
   * **实密结构体 (`forma densa`)**：强制无缝拼接，允许位域跨越字节边界（跨界拼接），这在解析紧凑的网络协议头时极为有用。
3. **取址限制**：**绝对禁止**对位域成员使用 `locus`（取地址）操作符。因为现代 CPU 无法生成指向非字节对齐地址的指针。
4. **结构体总大小与数组排布 (Magnitudo et Acies)**：无论内部位域如何紧凑拼接，结构体的总大小（`magnitudo`）最终必定向上取整到最接近的**整数字节**。因此，当带有位域的 `forma densa` 被放入数组（`acies`）时，数组的每个元素必定从字节边界开始，绝不会出现跨元素的比特级拼接。例如，内部位域总计 13 bits 的 `forma densa`，其 `magnitudo` 为 2 字节，在数组中每个元素占用 2 字节。
